...
LVIS_COCO_CATEGORIES = openseg_classes.get_lvis_coco_categories_with_prompt_eng()

_PREDEFINED_SPLITS_LVIS_COCO_PANOPTIC = {
    "openvocab_lvis_v1_merge_coco_train_panoptic": "coco/annotations/lvis_v1_merge_coco_train.json",
    "openvocab_lvis_v1_merge_coco_val_panoptic": "coco/annotations/lvis_v1_merge_coco_val.json",
}

def get_metadata():
    """
    Generates metadata for the LVIS-COCO merge panoptic dataset.
    ...
    thing_classes = [k["name"] for k in LVIS_COCO_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in LVIS_COCO_CATEGORIES if k["isthing"] == 1]
    stuff_classes = [k["name"] for k in LVIS_COCO_CATEGORIES]
    stuff_colors = [k["color"] for k in LVIS_COCO_CATEGORIES]
    ...
    for i, cat in enumerate(LVIS_COCO_CATEGORIES):
        if cat["isthing"]:
...
def load_lvis_coco_panoptic_json(json_file, image_dir, meta):
    """
...
def register_lvis_coco_panoptic(
    ...
    DatasetCatalog.register(
        panoptic_name,
        lambda: load_lvis_coco_panoptic_json(panoptic_json, image_root, metadata),
    )
...
def register_all_lvis_coco_panoptic_annos(root):
    """
...
register_all_lvis_coco_panoptic_annos(_root)