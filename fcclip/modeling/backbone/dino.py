...
class DINOv3(Backbone):
    def __init__(self, cfg, input_shape):
        ...
        backbone_path = cfg.MODEL.FC_CLIP.DINOV3_BACKBONE_PATH
        # Path of the tokenizer.
        tokenizer_path = cfg.MODEL.FC_CLIP.DINOV3_TOKENIZER_PATH

        assert cfg.MODEL.FC_CLIP.EMBED_DIM == 2048, "Incorrect embedding dimension for DINOv3"
        ...
        visual_backbone = DinoVisionTransformer(
            img_size=224,
            patch_size=16,
            in_chans=3,
        ...
        self.visual_model = VisionTower(
            ...
            patch_token_layer=1, # last layer
            patch_tokens_pooler_type="mean",
            use_linear_projection=False,
        )

        self.logit_scale = nn.Parameter(torch.empty(1))
        torch.nn.init.constant_(self.logit_scale, math.log(1 / 0.07))
        ...
        self._out_feature_strides = {
            "res4": self.visual_model.backbone.patch_embed.patch_size[0], # 16
            "clip_vis_dense": self.visual_model.backbone.patch_embed.patch_size[0], # 16
            "clip_embedding": -1
        }
    ...
    def extract_features(self, x):
        ...
        cls_token, patch_tokens, backbone_patch_tokens = self.visual_model(x)

        # Turn patch tokens from (B,H*W,C) to (B,C,H,W)
        B,_,Hf,Wf = x.shape
        H = Hf // self.visual_model.backbone.patch_embed.patch_size[0]
        W = Wf // self.visual_model.backbone.patch_embed.patch_size[1]
        patch_tokens = patch_tokens.transpose(1,2).view(B,-1,H,W)

        out['res4'] = patch_tokens.contiguous() # os16
        out['clip_vis_dense'] = patch_tokens.contiguous() # os16
        out['clip_embedding'] = cls_token.contiguous()
    ...
    @property
    def dim_latent(self):
        return self._out_feature_channels["clip_embedding"]
    ...
    def output_shape(self):
        return {
            ...
            for name in ["res4", "clip_vis_dense", "clip_embedding"]
        }
